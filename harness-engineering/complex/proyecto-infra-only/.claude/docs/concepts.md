...
explorar → proponer → especificar/diseñar → planificar tareas →
implementar → revisar → verificar → archivar.
...
- **GATE#1** post-Proposer: elige entre alternativas técnicas.
- **GATE#2** post-Code-reviewer: aprueba el código antes de verify.
- **GATE#3** sólo en perfiles con `gate3_pre_apply` (típicamente
  `infra-only`): pre-`apply` de operaciones irreversibles.
...
Este proyecto usa **flow_type = `complex`** declarado en

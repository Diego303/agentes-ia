...
>
> **Configuración actual**: flow_type = `complex`, perfil = `fullstack-web`.
...
Este proyecto usa flujo COMPLEX: 9 agentes canónicos con fases especializadas
(explorer → proposer → spec-writer ‖ designer → task-planner → implementer →
code-reviewer → verifier → archiver).
...
- `dominios`: software, frontend, content, docs

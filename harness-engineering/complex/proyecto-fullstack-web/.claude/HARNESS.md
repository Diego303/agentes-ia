...
- **Perfil**: fullstack-web
- **Cliente principal**: claude-code
- **Bundle directory**: `.claude/`
- **Tipo de flujo**: complex (9 agentes canónicos)
- **Dominios activos**: software, frontend, content, docs
...
Explorer → Proposer → GATE#1 → (Spec-writer ‖ Designer) → Task-planner
                                                                ↓
Archiver ← Verifier ← GATE#2 ← Code-reviewer ← Implementer
```
...
| 10 | archive | archiver | todo el SDD/ | SDD/spec.lock.yaml, actualiza feature_list | DONE |
...
- `security-critical`, `auth`, `crypto` → invoca `security-auditor` antes de code-reviewer.
- `compliance`, `gdpr`, `audit` → invoca `compliance-officer` en gate2.
- `iac`, `terraform` → fase `verify` ejecuta también `terraform-planner` antes del `verifier`.
...
→ Próximo: proposer (sonnet, lee SDD/context.md, escribirá SDD/proposal.md)
...
### GATE #1 (post-Proposer)

El humano elige entre las 2-3 alternativas de `SDD/proposal.md`.
Sintaxis de aprobación:

- Claude Code: `/approve <alt-id>` o `/reject <razón>`.
- Otros clientes: respuesta en texto plano "apruebo alt-B" o "rechazo, motivo: ...".

### GATE #2 (post-Code-reviewer)

El humano revisa el diff y `SDD/review.md`.
Sintaxis igual.

### GATE #3 (solo perfiles infra, pre-apply)

Antes de `terraform apply`. Humano revisa el plan.

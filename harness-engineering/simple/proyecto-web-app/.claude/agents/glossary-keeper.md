...
description: Mantiene docs/glossary.md y anexa términos nuevos a archive.md. Usar
  cuando Archiver terminó y la feature introdujo terminología nueva.
...
6. Anota en `<bundle>/state/<id>/SDD/archive.md` los términos añadidos.
...
```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "glossary-keeper", "event": "started"}
```

Al terminar exitosamente:

```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "glossary-keeper", "event": "completed", "artifact": "<path-relativo>", "duration_ms": <N>}
```

Si fallas:

```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "glossary-keeper", "event": "failed", "error": "<mensaje>"}
```

Tu `agent` ID debe ser exactamente `glossary-keeper` (matching el `name:` de este archivo).
...
- Sub-sección **`## Glossary updates`** anexada a
  `<bundle>/state/<id>/SDD/archive.md` con los términos añadidos.

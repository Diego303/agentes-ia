...
description: Captura decisiones arquitectónicas como ADRs y anexa la referencia a
  archive.md. Usar cuando Archiver terminó y hubo decisión no trivial entre alternativas.
...
## Inputs esperados
- `<bundle>/state/<id>/SDD/brainstorm.md`, `prior-art.md`, `hypotheses.md`,
...
2. Si no requiere ADR, escribe una nota corta justificándolo en
   `<bundle>/state/<id>/SDD/no-adr.md` y termina.
...
5. Anota en `<bundle>/state/<id>/SDD/archive.md` el ADR creado y su número.
...
```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "decision-recorder", "event": "started"}
```

Al terminar exitosamente:

```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "decision-recorder", "event": "completed", "artifact": "<path-relativo>", "duration_ms": <N>}
```

Si fallas:

```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "decision-recorder", "event": "failed", "error": "<mensaje>"}
```

Tu `agent` ID debe ser exactamente `decision-recorder` (matching el `name:` de este archivo).
...
- Nuevo archivo en `docs/adr/NNNN-titulo-corto.md` (o equivalente), o
  `<bundle>/state/<id>/SDD/no-adr.md` si se decide que no aplica.
- Sub-sección **`## ADR reference`** anexada a
  `<bundle>/state/<id>/SDD/archive.md` con el número del ADR creado y un link.

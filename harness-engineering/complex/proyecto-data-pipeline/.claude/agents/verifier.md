---
name: verifier
description: Ejecuta verify.sh y reporta pass/fail con evidencia. Usar cuando GATE#2
  está aprobado y antes de archiver.
...
- `verify_stack` del perfil (p. ej. `ruff`, `pytest`, `secret-scanning`,
  `markdownlint`).
- `<bundle>/state/<feature-id>/SDD/requirements.md` para validar criterios de aceptación.
...
```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "verifier", "event": "started"}
```

Al terminar exitosamente:

```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "verifier", "event": "completed", "artifact": "<path-relativo>", "duration_ms": <N>}
```

Si fallas:

```jsonl
{"timestamp": "<ISO-8601-UTC>", "feature_id": "<id>", "agent": "verifier", "event": "failed", "error": "<mensaje>"}
```

Tu `agent` ID debe ser exactamente `verifier` (matching el `name:` de este archivo).
Esto es crítico para que `harness events` verifique multi-agencia.

## Output
- `<bundle>/state/<feature-id>/SDD/verification.md` con secciones: Comandos
